...
<head>
    ...
    <style>
        ...
        .glass-card { 
            ...
            box-shadow: 0 15px 35px -5px rgba(30, 64, 175, 0.15), inset 0 0 15px rgba(255, 255, 255, 0.5);
        }
...
<body class="p-4 md:p-10 text-slate-800">
    <div class="max-w-7xl mx-auto">
        ...
        <div class="mb-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
            ...
            <div class="text-left md:text-right bg-white/50 px-4 py-2 rounded-xl border border-white/60 shadow-sm backdrop-blur-md">
                <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Data Version</p>
                <p class="text-xs font-black text-slate-600">LIVE W<?= $device['report_week'] ?> / <?= $device['report_year'] ?></p>
            </div>
        </div>

        <!-- Hero Section -->
        <div class="bg-white/90 backdrop-blur-xl rounded-[2rem] p-8 shadow-2xl shadow-blue-900/10 border border-white mb-8 flex flex-col md:flex-row justify-between items-center gap-6">
            <div class="flex-1 w-full">
                <div class="flex flex-wrap items-center gap-3 mb-3">
                    <span class="px-3 py-1 bg-slate-800 text-white rounded-full text-[9px] font-black uppercase tracking-widest shadow-sm"><?= htmlspecialchars($device['computer_type']) ?></span>
                    <span class="px-3 py-1 bg-blue-600 text-white rounded-full text-[9px] font-black uppercase tracking-widest shadow-sm border border-blue-500"><?= htmlspecialchars($device['bu']) ?></span>
                    <?php 
                    ...
                    ?>
                    <span class="px-3 py-1 <?= $hero_badge_bg ?> border rounded-full text-[9px] font-black uppercase tracking-widest flex items-center gap-1.5 shadow-sm">
                        <?php if ($is_inactive_hero): ?>
                            <span class="w-2 h-2 bg-rose-500 rounded-full animate-pulse shadow-[0_0_8px_rgba(244,63,94,0.6)]"></span>
                        <?php else: ?>
                ...
                <p class="text-sm font-bold text-slate-500 flex items-center gap-2">Serial No: <span class="bg-slate-100/80 text-slate-600 px-2.5 py-0.5 rounded-lg border border-slate-200 text-xs font-black"><?= htmlspecialchars($device['serial_no']) ?></span></p>
            </div>
            <div class="w-full md:w-auto flex flex-col gap-4">
                <div class="bg-slate-50/80 p-6 rounded-[1.5rem] border border-slate-100 text-center min-w-[170px] shadow-sm relative overflow-hidden backdrop-blur-sm">
                    <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1 relative z-10">Compliance Score</p>
                    <?php 
                    ...
                    <p class="text-5xl font-black <?= $score_color ?> relative z-10 drop-shadow-sm"><?= $score_pct ?>%</p>
                    <p class="text-[9px] font-black text-slate-400 mt-1 uppercase tracking-tighter relative z-10 bg-white/60 mx-auto px-2 py-0.5 rounded-full w-fit"><?= $passed_count ?> OF 6 PASSED</p>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            <!-- User & Location -->
            <div class="glass-card relative rounded-[2.5rem] p-8 shadow-xl hover:-translate-y-2 hover:shadow-2xl hover:shadow-blue-500/20 transition-all duration-300 group">
                <!-- Hover Indicator -->
            ...
            <!-- OS Details -->
            <div class="glass-card relative rounded-[2.5rem] p-8 shadow-xl hover:-translate-y-2 hover:shadow-2xl hover:shadow-indigo-500/20 transition-all duration-300 group">
                <!-- Hover Indicator -->
                <div class="absolute top-6 right-6 w-10 h-10 bg-gradient-to-tr from-white/60 to-white/20 rounded-full border border-white shadow-lg flex items-center justify-center opacity-0 group-hover:opacity-100 transform scale-50 group-hover:scale-100 transition-all duration-500 pointer-events-none z-10">
            ...
            <!-- Security Compliance -->
            <div class="glass-card relative rounded-[2.5rem] p-8 shadow-xl hover:-translate-y-2 hover:shadow-2xl hover:shadow-emerald-500/20 transition-all duration-300 group">
                <!-- Hover Indicator -->
                <div class="absolute top-6 right-6 w-10 h-10 bg-gradient-to-tr from-white/60 to-white/20 rounded-full border border-white shadow-lg flex items-center justify-center opacity-0 group-hover:opacity-100 transform scale-50 group-hover:scale-100 transition-all duration-500 pointer-events-none z-10">
            ...
            <!-- Admin & Network -->
            <div class="glass-card relative rounded-[2.5rem] p-8 shadow-xl hover:-translate-y-2 hover:shadow-2xl hover:shadow-slate-500/20 transition-all duration-300 group">
                <!-- Hover Indicator -->
                <div class="absolute top-6 right-6 w-10 h-10 bg-gradient-to-tr from-white/60 to-white/20 rounded-full border border-white shadow-lg flex items-center justify-center opacity-0 group-hover:opacity-100 transform scale-50 group-hover:scale-100 transition-all duration-500 pointer-events-none z-10">
            ...
            <!-- Maintenance & Uptime -->
            <div class="glass-card relative rounded-[2.5rem] p-8 shadow-xl hover:-translate-y-2 hover:shadow-2xl hover:shadow-orange-500/20 transition-all duration-300 group">
                <!-- Hover Indicator -->
                <div class="absolute top-6 right-6 w-10 h-10 bg-gradient-to-tr from-white/60 to-white/20 rounded-full border border-white shadow-lg flex items-center justify-center opacity-0 group-hover:opacity-100 transform scale-50 group-hover:scale-100 transition-all duration-500 pointer-events-none z-10">
                ...
                <?php 
                    $reboot_display = 'N/A';
                    if ($device['last_reboot_date'] && $device['last_reboot_date'] !== 'N/A') {
                        $reboot_display = date('d M Y, H:i', strtotime($device['last_reboot_date']));
                    }
                ?>
                ...
                <div class='flex justify-between items-center py-3'>
                    <span class='text-[11px] font-bold text-slate-400 uppercase tracking-wider'>GLPI Agent Status</span>
                    <?php 
                        $glpi_bg = ($device['glpi_agent_status'] == 'Installed') ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600';
                    ?>
                    <span class='text-[10px] font-black uppercase tracking-widest <?= $glpi_bg ?> px-3 py-1 rounded-full border border-white shadow-sm'><?= htmlspecialchars($device['glpi_agent_status']) ?></span>
                </div>
            </div>

            <!-- Patches & Admin Groups -->
            <div class="glass-card relative rounded-[2.5rem] p-8 shadow-xl hover:-translate-y-2 hover:shadow-2xl hover:shadow-slate-400/20 transition-all duration-300 group bg-slate-50/50">
                <!-- Hover Indicator -->
                <div class="absolute top-6 right-6 w-10 h-10 bg-gradient-to-tr from-white/60 to-white/20 rounded-full border border-white shadow-lg flex items-center justify-center opacity-0 group-hover:opacity-100 transform scale-50 group-hover:scale-100 transition-all duration-500 pointer-events-none z-10">
                    <svg class="w-4 h-4 text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M12 6V4m0 2a2 2 0 100 4m0-4a2 2 0 110 4m-6 8a2 2 0 100-4m0 4a2 2 0 110-4m0 4v2m0-6V4m6 6v10m6-2a2 2 0 100-4m0 4a2 2 0 110-4m0 4v2m0-6V4"></path></svg>
                </div>
                ...
                <div class="mb-4">
                    <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 flex justify-between">
                        Missing Patches
                        <span class="bg-rose-100 text-rose-600 px-2 py-0.5 rounded-full shadow-inner"><?= htmlspecialchars($device['total_missing_patches']) ?> items</span>
                    </p>
                    <div class="bg-white p-4 rounded-2xl border border-slate-100 text-[10px] font-mono text-slate-500 max-h-24 overflow-y-auto shadow-inner leading-relaxed">
                        <?= htmlspecialchars($device['missing_patches_name'] ?: 'None') ?>
                    </div>
                </div>
                <div>
                    <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Admin Members</p>
                    <div class="bg-white p-4 rounded-2xl border border-slate-100 text-[10px] font-mono text-slate-500 max-h-20 overflow-y-auto shadow-inner leading-relaxed">
