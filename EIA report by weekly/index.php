...
$latest_date = getLatestReportWeek($pdo);
$week = $latest_date['week'];
$year = $latest_date['year'];

// 3. Fetch Metrics
// Grand Total
$current_total = getCurrentTotal($pdo, $week, $year);
...
foreach ($topics_map as $label => $col) {
    $s = getTopicSuccessCount($pdo, $week, $year, $col);
    $p = $current_total - $s;
...
// Serviced By Data
$serviced_data = getServicedByDensity($pdo, $week, $year);

// Top BUs (Patch Success)
$bu_data = getTopBUPatchSuccess($pdo, $week, $year);
