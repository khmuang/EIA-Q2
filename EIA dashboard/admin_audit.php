...
$cache_file = 'admin_audit_cache.json';
$cache_lifetime = 10800; // 180 minutes
$lock_file = 'admin_audit.lock';
$data_from_cache = false;
$needs_update = false;

// 1. Check if cache exists (load even if stale)
...
if (!file_exists($cache_file)) {
    ...
    if (!file_exists($lock_file)) {
        ...
        $cmd = "start /B $php_path \"$worker_path\" --type=admin_audit > NUL 2>&1";
        exec($cmd);
    }
}

if ($needs_update && !$data_from_cache) {
    // Show 'Preparing Data' ONLY if we have absolutely no cache
    $all_items = []; 
...
if ($search !== '' || $filter_bu !== '') {
    $filtered_items = array_filter($all_items, function($item) use ($search, $filter_bu) {
        ...
        if ($search !== '') {
            ...
            $match_search = (
                strpos(strtolower($item['Computer Name']), $search_lc) !== false || 
                strpos(strtolower($item['Serial no.']), $search_lc) !== false ||
                strpos(strtolower($item['Full Name']), $search_lc) !== false
            );
        }
        
        if ($filter_bu !== '') {
            $match_bu = ($item['BU'] === $filter_bu);
        }
...
<head>
    ...
    <title>Standard Admin Rights | EIA Dashboard</title>
    ...
    <div class="max-w-[1600px] mx-auto">
        <div class="glass-card rounded-xl overflow-hidden shadow-2xl border border-white mb-6">
            <div class="p-6 md:p-8 flex flex-col md:flex-row justify-between items-center gap-6 border-b border-white/40 bg-white/30 relative overflow-hidden">
                <div class="img-blur"><img src="Image/bg-computer-2.jpg" alt="BG" class="w-full h-full object-cover"></div>
                <div class="relative z-10 flex items-center gap-4">
                    ...
                    <div>
                        <h1 class="text-3xl lg:text-4xl font-black text-slate-800 dark:text-white tracking-tight" style="font-family: 'Montserrat', sans-serif;">Standard Admin Rights</h1>
                        <div class="flex items-center gap-2 mt-0.5"><span class="w-2 h-2 rounded-full bg-rose-500 animate-pulse"></span><span class="text-xs font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest">Privilege Risk Monitoring</span></div>
                    </div>
                </div>
                <div class="flex flex-wrap items-center gap-3 justify-end relative z-10 text-right">
                    ...
                    <div class="text-xs font-black text-rose-600 bg-rose-50 dark:bg-rose-900/20 px-4 py-2 rounded-xl border border-rose-100 dark:border-rose-800/50 shadow-inner uppercase tracking-widest">Found: <?= number_format($total_filtered) ?> Units</div>
                    <a href="index.php" class="bg-blue-600 px-4 py-2 rounded-xl shadow-sm border border-blue-500 text-white font-bold flex items-center gap-2 hover:bg-blue-700 transition-all group text-xs uppercase tracking-widest active:scale-95">
            ...
            <div class="px-6 py-4 bg-white/50 border-b border-gray-100 flex flex-wrap items-center gap-4">
                <form method="GET" class="flex flex-wrap items-center gap-3 w-full">
                    ...
                    <button type="submit" class="bg-slate-800 text-white px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-slate-900 transition-all shadow-md">Apply Filters</button>
                    
                    <?php if ($search !== '' || $filter_bu !== ''): ?>
                        <a href="admin_audit.php" class="bg-rose-50 text-rose-600 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-rose-600 hover:text-white transition-all border border-rose-100 shadow-sm ml-2">
                            Clear Filters
                        </a>
                    <?php endif; ?>
                </form>
                ...
                <table class="w-full text-left whitespace-nowrap border-separate border-spacing-0">
                    <thead>
                        <tr>
                            ...
                            <th class="px-6 py-5 text-xs font-black uppercase tracking-wider text-center border-r border-slate-200 dark:border-slate-700">Status</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-700 bg-white/40 dark:bg-slate-800/20">
                        ...
                        <tr class="hover-row transition-all duration-300 group">
                            <td class="px-6 py-4">
                                <a href="inventory_detail.php?name=<?= urlencode($row['Computer Name']) ?>&back=<?= urlencode($_SERVER['REQUEST_URI']) ?>" class="font-bold text-slate-700 dark:text-slate-300 hover:text-indigo-600 flex items-center gap-2 transition-colors tracking-tight uppercase text-xs">
                            ...
                            <td class="px-6 py-4 text-center"><span class="px-3 py-1 bg-rose-100 text-rose-600 rounded-full text-xs font-black uppercase tracking-widest border border-rose-200">Non-Standard</span></td>
                            </tr>
                            <?php endforeach; ?>
                            <?php if (empty($items)): ?>
                            <tr><td colspan="7" class="px-6 py-24 text-center text-slate-400 italic font-bold">No results found matching your criteria.</td></tr>
                            <?php endif; ?>
                            </tbody>
                            </table>
                            </div>
                            ...
                            Showing <?= min($offset + 1, $total_filtered) ?> - <?= min($offset + $limit, $total_filtered) ?> of <?= number_format($total_filtered) ?>
                            </div>
                            ...
                            ?>
                            <?php if ($page > 1): ?>
                            <a href="<?= get_page_url($page - 1, $query_params) ?>" class="px-4 py-2 bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-xl text-xs font-black text-slate-500 dark:text-slate-300 hover:bg-slate-50 transition-all uppercase">Prev</a>
                            <?php endif; ?>
                            <div class="flex items-center px-5 text-xs font-black text-white bg-blue-600 rounded-xl shadow-lg border border-blue-500">
                            PAGE <?= $page ?> / <?= $total_pages ?>
                            </div>
                            <?php if ($page < $total_pages): ?>
                            <a href="<?= get_page_url($page + 1, $query_params) ?>" class="px-4 py-2 bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-xl text-xs font-black text-slate-500 dark:text-slate-300 hover:bg-slate-50 transition-all uppercase">Next</a>
                            <?php endif; ?>
                            </div>
                            </div>
                            </div>
                            <footer class="mt-12 text-center pb-12"><p class="text-xs font-black text-slate-400 uppercase tracking-widest">EIA Dashboard &copy; 2026 Prepared by Endpoint Management Team</p></footer>
                            </div>
    <!-- Back to Top Button -->
