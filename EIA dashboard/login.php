...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $username_for_ad = $raw_username;
    if (($pos = strpos($username_for_ad, "@")) !== false) {
        $username_for_ad = substr($username_for_ad, 0, $pos);
    }
    if (($pos = strpos($username_for_ad, "\\")) !== false) {
        $username_for_ad = substr($username_for_ad, $pos + 1);
    }

    $username_lc = strtolower($username_for_ad);

    // Require LDAP Helper
    require_once 'adauthen.php';

    if (function_exists('chkldapuser')) {
        // Optimized: Single Domain Authentication (Central)
        $chkldapresult = "Invalid";
        $full_name = "";

        $res = chkldapuser('central', $username_for_ad, $password);
        
        if (is_array($res) && $res['status'] === "Pass") {
            $chkldapresult = "Pass";
            $full_name = $res['full_name'];
        } elseif ($res === "Pass") {
            $chkldapresult = "Pass";
        } else {
            $chkldapresult = is_array($res) ? $res['status'] : $res;
        }

        switch ($chkldapresult) {
            case "Pass":
                // --- SECOND LAYER: Whitelist Check ---
                $whitelist_file = 'authorized_users.txt';
                $is_authorized = false;
                
                if (file_exists($whitelist_file)) {
                    $authorized_list = file($whitelist_file, FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES);
                    foreach ($authorized_list as $line) {
                        $clean_entry = strtolower(trim($line));
                        if (empty($clean_entry) || strpos($clean_entry, '#') === 0) continue;
                        
                        if ($clean_entry === $username_lc) {
                            $is_authorized = true;
                            break;
                        }
                    }
                } else {
                    $error = 'ระบบความปลอดภัยขัดข้อง (Whitelist file missing)';
                    break;
                }

                if ($is_authorized) {
                    $_SESSION['logged_in'] = true;
                    $_SESSION['username'] = $username_for_ad;
                    $_SESSION['full_name'] = $full_name; // STORE FULL NAME
                    
                    // session_regenerate_id(true); // Removed for stability on production
                    session_write_close();
                    
                    header('Location: index.php', true, 302);
                    exit;
                } else {
                    $error = 'คุณไม่มีสิทธิ์เข้าใช้งานระบบ (Unauthorized Access)';
                }
                break;
            case "Not found":
            ...
            case "Invalid":
                ...
                break;
            default:
                $error = 'เกิดข้อผิดพลาดในการยืนยันตัวตน: ' . $chkldapresult;
        }
    } else {
        $error = 'ระบบ LDAP ขัดข้อง หรือติดต่อไม่ได้';
    }
}
?>
<!DOCTYPE html>
...
<body>
    ...
    <div class="glass-card w-full max-w-lg rounded-3xl p-10 md:p-14 relative overflow-hidden m-4">
        ...
        <div class="text-center mb-10 relative z-10">
            ...
            <p class="text-xs text-gray-500 font-bold uppercase tracking-widest">Authentication Required</p>
        </div>
        ...
        <form method="POST" action="login.php" class="space-y-6 relative z-10">
            ...
            <div class="pt-2">
                <button type="submit" id="login-btn"
                    ...
                    <span id="btn-text">Secure Login &rarr;</span>
                </button>
            </div>
            ...
            <script>
                document.querySelector('form').addEventListener('submit', function(e) {
                    ...
                    text.innerHTML = '<span class="flex items-center justify-center gap-3"><svg class="animate-spin h-4 w-4 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg> Authenticating...</span>';
                });
            </script>
            
            <p class="text-center text-xs text-gray-500 font-medium uppercase tracking-widest mt-8">Authorized Personnel Only</p>
        </form>
    </div>
