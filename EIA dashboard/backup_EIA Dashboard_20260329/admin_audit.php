...
if (file_exists($cache_file) && (time() - filemtime($cache_file) < $cache_lifetime)) {
    ...
    if ($cache_data) {
        ...
        $total_risk = $cache_data['total_risk'];
...
if (!file_exists($cache_file) || (time() - filemtime($cache_file) > $cache_lifetime)) {
    if (!file_exists($lock_file)) {
        ...
        pclose(popen("start /B $php_path cache_worker.php --type=admin_audit", "r"));
    }
}
...
if (!$data_from_cache) {
    ...
    try {
        // Fetch All Risk Units (No Limit for Caching)
    ...
    } catch (\PDOException $e) { $all_items = []; $total_risk = 0; $last_updated = "Error"; }
}
...
<head>
    ...
    <title>EIA Compliance Dashboard — Standard Admin Rights</title>
        ...
        <div class="glass-card rounded-xl overflow-hidden shadow-2xl border border-white mb-10">
            <div class="p-6 md:p-8 flex flex-col md:flex-row justify-between items-center gap-6 border-b border-white/40 bg-white/30 relative overflow-hidden">
                ...
                <div class="flex flex-wrap items-center gap-3 justify-end relative z-10 text-right">
                    ...
                    <div class="text-[10px] font-black text-rose-600 bg-rose-50 dark:bg-rose-900/20 px-4 py-2 rounded-xl border border-rose-100 dark:border-rose-800/50 shadow-inner uppercase tracking-widest">High Risk: <?= number_format($total_risk) ?> Units</div>
            ...
            <div class="overflow-x-auto max-h-[650px]">
                <table class="w-full text-left whitespace-nowrap border-separate border-spacing-0">
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-700 bg-white/40 dark:bg-slate-800/20">
                        ...
                        <?php endforeach; ?>
                        <?php if (empty($items)): ?>
                        <tr><td colspan="7" class="px-6 py-24 text-center text-slate-400 italic">No admin rights issues found.</td></tr>
                        <?php endif; ?>
                    </tbody>
                </table>
            </div>
