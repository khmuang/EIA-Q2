...
if (!file_exists($cache_file)) {
    ...
    if (!file_exists($lock_file)) {
        ...
        $cmd = "start /B $php_path \"$worker_path\" --type=inactive_assets > NUL 2>&1";
        exec($cmd);
    }
}
...
<head>
    ...
    <title>Inactive Assets | EIA Dashboard</title>
    ...
    <div class="max-w-[1600px] mx-auto">
        <div class="glass-card rounded-xl overflow-hidden shadow-2xl border border-white mb-6">
            <div class="p-6 md:p-8 flex flex-col md:flex-row justify-between items-center gap-6 border-b border-white/40 bg-white/30 relative overflow-hidden">
                <div class="img-blur"><img src="Image/bg-computer-3.jpg" alt="BG" class="w-full h-full object-cover"></div>
                <div class="relative z-10 flex items-center gap-4">
                    ...
                    <div>
                        <h1 class="text-3xl lg:text-4xl font-black text-slate-800 dark:text-white tracking-tight" style="font-family: 'Montserrat', sans-serif;">Inactive Assets</h1>
                        <div class="flex items-center gap-2 mt-0.5"><span class="w-2 h-2 rounded-full bg-amber-500 animate-pulse"></span><span class="text-xs font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest">Asset Lifecycle Management</span></div>
                    </div>
                </div>
                <div class="flex flex-wrap items-center gap-3 justify-end relative z-10 text-right">
                    ...
                    <div class="text-xs font-black text-amber-600 bg-amber-50 dark:bg-amber-900/20 px-4 py-2 rounded-xl border border-amber-100 dark:border-amber-800/50 shadow-inner uppercase tracking-widest">Found: <?= number_format($total_filtered) ?> Units</div>
            ...
            <div class="px-6 py-4 bg-white/50 border-b border-gray-100 flex flex-wrap items-center gap-4">
                <form method="GET" class="flex flex-wrap items-center gap-3 w-full">
                    ...
                    <?php if ($search !== '' || $filter_bu !== ''): ?>
                        <a href="inactive_assets.php" class="bg-rose-50 text-rose-600 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-rose-600 hover:text-white transition-all border border-rose-100 shadow-sm ml-2">
                            Clear Filters
                        </a>
                    <?php endif; ?>
                </form>
                ...
                <table class="w-full text-left whitespace-nowrap border-separate border-spacing-0">
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-700 bg-white/40 dark:bg-slate-800/20">
                        ...
                        <tr class="hover-row transition-all duration-300 group">
                            <td class="px-6 py-4">
                                <a href="inventory_detail.php?name=<?= urlencode($row['Computer Name']) ?>&back=<?= urlencode($_SERVER['REQUEST_URI']) ?>" class="font-bold text-slate-700 dark:text-slate-300 hover:text-indigo-600 dark:hover:text-indigo-400 flex items-center gap-2 transition-colors tracking-tight uppercase text-xs">
                            ...
                            <td class="px-6 py-4 text-center"><span class="px-3 py-1 bg-amber-100 text-amber-600 rounded-full text-xs font-bold uppercase tracking-tight border border-amber-200">Inactive 30+ Days</span></td>
                            </tr>
                            <?php endforeach; ?>
                            <?php if (empty($items)): ?>
