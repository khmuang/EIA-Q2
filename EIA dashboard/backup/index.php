...
if ($needs_update || !$data_from_cache) {
    ...
    try {
        ...
        // Formula for Score Calculation (0-6)
        $calc_score_sql = "(
        ...
        // 1. Master Query for KPI Successes + 4 Compliance Grades
        $master_sql = "
            SELECT 
                COUNT(*) as total,
                SUM(CASE WHEN `Joined Approved Domain` IN $success_keys THEN 1 ELSE 0 END) as s_domain,
                SUM(CASE WHEN `OS End of Support Status` IN $success_keys THEN 1 ELSE 0 END) as s_os,
                SUM(CASE WHEN `Patch Healthy` IN $success_keys THEN 1 ELSE 0 END) as s_patch,
                SUM(CASE WHEN `Antivirus Compliant` IN $success_keys THEN 1 ELSE 0 END) as s_av,
                SUM(CASE WHEN `Firewall Compliant` IN $success_keys THEN 1 ELSE 0 END) as s_fw,
                SUM(CASE WHEN `Standard Admin Only` IN $success_keys THEN 1 ELSE 0 END) as s_admin,
                SUM(CASE WHEN `OS End of Support Status` = 'Ended' THEN 1 ELSE 0 END) as s_ended,
                SUM(CASE WHEN $calc_score_sql = 6 THEN 1 ELSE 0 END) as g_full,
                SUM(CASE WHEN $calc_score_sql = 5 THEN 1 ELSE 0 END) as g_minor,
                SUM(CASE WHEN $calc_score_sql BETWEEN 3 AND 4 THEN 1 ELSE 0 END) as g_at_risk,
                SUM(CASE WHEN $calc_score_sql BETWEEN 0 AND 2 THEN 1 ELSE 0 END) as g_critical
            FROM ($clean_query) as base_data
        ";
        $stmt_master = $pdo->query($master_sql);
        $master_data = $stmt_master->fetch(PDO::FETCH_ASSOC);
        
        $current_total = (int)$master_data['total'] ?: 1;
        $g_full = (int)$master_data['g_full'];
        $g_minor = (int)$master_data['g_minor'];
        $g_at_risk = (int)$master_data['g_at_risk'];
        $g_critical = (int)$master_data['g_critical'];
        $s_ended = (int)$master_data['s_ended'];

        $p_full = ($g_full / $current_total) * 100;
        $p_minor = ($g_minor / $current_total) * 100;
        $p_at_risk = ($g_at_risk / $current_total) * 100;
        $p_critical = ($g_critical / $current_total) * 100;
        $s1 = $p_full; $s2 = $s1 + $p_minor; $s3 = $s2 + $p_at_risk;

        // 2. KPI Results Mapping
        $topic_results = [];
        $total_success_sum = 0;
        ...
        foreach ($topics_mapping_keys as $label => $key) {
            ...
            // NEW LOGIC: If Patching, exclude s_ended from denominator for RATE
            if ($label === 'Patching') {
                $effective_total = $current_total - $s_ended;
                $rate = ($effective_total > 0) ? round(($s / $effective_total) * 100, 1) : 0;
                $note = "Excluded " . number_format($s_ended) . " OS Ended units";
            } else {
                $rate = round(($s / $current_total) * 100, 1);
                $note = "";
            }

            // P (Pending) should always be absolute difference to reflect total risk in alerts
            $p = $current_total - $s;
        ...
        // Save to Cache
        $cache_payload = compact('current_total','g_full','g_minor','g_at_risk','g_critical','p_full','p_minor','p_at_risk','p_critical','s1','s2','s3','topic_results','total_success_sum','overall_rate','serviced_data','bu_data','max_bu_val','last_updated','s_ended');
        file_put_contents($cache_file, json_encode($cache_payload));

    } catch (\PDOException $e) {
        die("<div style='color:red; font-family:sans-serif;'><strong>Database Query Failed:</strong> " . htmlspecialchars($e->getMessage()) . "</div>");
    }
}
...
<head>
    ...
    <!-- Font -->
    <link href="https://fonts.googleapis.com/css2?family=Montserrat:wght@400;500;600;700;800&family=Sarabun:wght@300;400;500;600;700&display=swap" rel="stylesheet">
    <style>
        body { 
    ...
    <div class="max-w-7xl mx-auto bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden relative">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center p-5 border-b border-gray-100 bg-white/50 backdrop-blur-md sticky top-0 z-[100] relative overflow-hidden">
            ...
            <div class="flex flex-wrap items-center md:items-start gap-3 text-sm w-full md:w-auto relative z-10">
                ...
                <a href="inventory_list.php" class="bg-gray-100 px-4 py-2 rounded-lg border hover:bg-gray-200 transition-colors flex items-center gap-2 font-bold text-xs text-gray-600 uppercase shadow-sm">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z"></path></svg>
                    Full List
                </a>
                <a href="https://rscsassetsrv02.central.co.th/cgitasset/login.php" target="_blank" class="bg-white px-4 py-2 rounded-lg border hover:bg-blue-50 hover:border-blue-200 text-blue-600 transition-colors flex items-center gap-2 font-bold text-xs uppercase shadow-sm">
        ...
        <div class="p-5 md:p-8 bg-slate-50">
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-5 mb-8">
                ...
                <a href="inventory_list.php?topic=<?= urlencode($topic['label']) ?>&status=success" class="block <?= $topic['bg_tail'] ?> rounded-xl border shadow-sm relative overflow-hidden p-5 hover:shadow-lg hover:-translate-y-1 transition-all duration-300 group">
                    ...
                    <div class="text-[10px] text-gray-400 font-bold mt-1"><?= number_format($topic['success']) ?> Units</div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8 items-stretch">
                ...
                <div class="lg:col-span-2 border rounded-[2rem] p-6 shadow-sm flex flex-col justify-between bg-blue-50 border-blue-100">
                    <div>
                        <div class="flex justify-between items-center mb-4">
                            <h3 class="text-sm font-black text-blue-700 tracking-[0.3em] uppercase flex items-center gap-3">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-4">
                            ...
                            <a href="inventory_list.php?area=<?= urlencode($area['serviced_by']) ?>" class="group block p-3 -m-3 rounded-2xl hover:bg-white/80 hover:shadow-xl hover:shadow-blue-200/50 transition-all duration-300 hover:-translate-y-1 active:scale-95">
                                <div class="flex justify-between items-end mb-2 px-1">
                                    <div class="text-[13px] font-black text-slate-800 truncate group-hover:text-blue-600 transition-colors uppercase pr-2"><?= htmlspecialchars($area['serviced_by']) ?></div>
                                ...
                                <div class="text-[9px] font-black text-slate-600 uppercase tracking-widest mt-1.5 bg-transparent"><?= round($pct, 1) ?>% Density</div>
                            </a>
                            <?php endforeach; ?>
                        </div>
                    </div>
                ...
                <a href="top_bu_compliance.php" class="lg:col-span-1 border rounded-[2.5rem] p-6 shadow-2xl hover:shadow-blue-200/50 hover:-translate-y-1 transition-all duration-500 group flex flex-col items-center justify-center text-center relative overflow-hidden border-white/20 min-h-[220px]" 
                   style="background: linear-gradient(135deg, #1e3a8a 0%, #3b82f6 100%);">
                    ...
                    <h3 class="text-lg font-black text-white uppercase tracking-tighter mb-1.5">Full BU Ranking</h3>
                    ...
                    <div class="mt-4 flex items-center gap-3 bg-white text-blue-700 px-5 py-2 rounded-xl font-black text-[10px] uppercase tracking-widest group-hover:bg-yellow-400 group-hover:text-slate-900 transition-all shadow-lg">
                        Explore
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                ...
                <div class="bg-white/60 border rounded-xl p-6 shadow-sm flex flex-col relative overflow-hidden backdrop-blur-md">
                    ...
                    <div class="flex justify-between items-center mb-6 relative z-10">
                        ...
                        <span class="text-[10px] font-black text-emerald-500 bg-emerald-50 px-2 py-0.5 rounded uppercase border border-emerald-100 shadow-sm"><span class="w-1.5 h-1.5 bg-emerald-500 rounded-full inline-block mr-1 animate-pulse"></span>Live</span>
                    </div>
                    <div class="flex flex-col items-center justify-center flex-1 relative z-10">
                        <div class="relative w-44 h-44 rounded-full flex items-center justify-center conic-chart shadow-xl border-4 border-white ring-1 ring-gray-100 transition-transform duration-500 hover:scale-105 group">
                            <div class="w-32 h-32 bg-white rounded-full flex flex-col items-center justify-center shadow-inner relative z-10">
                                ...
                                <span class="text-[9px] text-gray-400 font-black uppercase tracking-widest">Assets</span>
                            </div>
                        </div>
                        <div class="mt-8 space-y-2.5 w-full">
                            ...
                            <a href="inventory_list.php?score=100" class="flex items-center justify-between p-2.5 hover:bg-emerald-50 border border-transparent hover:border-emerald-100 rounded-xl transition-all group">
                                <div class="flex items-center gap-3">
                                    ...
                                    <span class="text-[11px] font-black text-gray-600 group-hover:text-emerald-700 uppercase tracking-tighter">Full Compliant (100%)</span>
                                </div>
                                <div class="text-right">
                                    <span class="text-xs font-black text-gray-800"><?= number_format($g_full) ?></span>
                                    <span class="text-[9px] font-bold text-gray-400 ml-1"><?= round($p_full, 1) ?>%</span>
                                </div>
                            </a>
                            <!-- Minor Issue -->
                            <a href="inventory_list.php?score=80-99" class="flex items-center justify-between p-2.5 hover:bg-blue-50 border border-transparent hover:border-blue-100 rounded-xl transition-all group">
                                <div class="flex items-center gap-3">
                                    <div class="w-2.5 h-2.5 bg-blue-500 rounded-full shadow-[0_0_8px_rgba(59,130,246,0.4)]"></div>
                                    <span class="text-[11px] font-black text-gray-600 group-hover:text-blue-700 uppercase tracking-tighter">Minor Issue (80-99%)</span>
                                </div>
                                <div class="text-right">
                                    <span class="text-xs font-black text-gray-800"><?= number_format($g_minor) ?></span>
                                    <span class="text-[9px] font-bold text-gray-400 ml-1"><?= round($p_minor, 1) ?>%</span>
                                </div>
                            </a>
                            <!-- At Risk -->
                            <a href="inventory_list.php?score=50-79" class="flex items-center justify-between p-2.5 hover:bg-amber-50 border border-transparent hover:border-amber-100 rounded-xl transition-all group">
                                <div class="flex items-center gap-3">
                                    ...
                                    <span class="text-[11px] font-black text-gray-600 group-hover:text-amber-700 uppercase tracking-tighter">At Risk (50-79%)</span>
                                </div>
                                <div class="text-right">
                                    <span class="text-xs font-black text-gray-800"><?= number_format($g_at_risk) ?></span>
                                    <span class="text-[9px] font-bold text-gray-400 ml-1"><?= round($p_at_risk, 1) ?>%</span>
                                </div>
                            </a>
                            <!-- Critical -->
                            <a href="inventory_list.php?score=<50" class="flex items-center justify-between p-2.5 hover:bg-rose-50 border border-transparent hover:border-rose-100 rounded-xl transition-all group">
                                <div class="flex items-center gap-3">
                                    <div class="w-2.5 h-2.5 bg-rose-500 rounded-full shadow-[0_0_8px_rgba(244,63,94,0.4)]"></div>
                                    <span class="text-[11px] font-black text-gray-600 group-hover:text-rose-700 uppercase tracking-tighter">Critical (<50%)</span>
                                </div>
                                <div class="text-right">
                                    <span class="text-xs font-black text-gray-800"><?= number_format($g_critical) ?></span>
                                    <span class="text-[9px] font-bold text-gray-400 ml-1"><?= round($p_critical, 1) ?>%</span>
                                </div>
                            </a>
                        </div>
                    </div>
                ...
                <div class="bg-white border rounded-xl p-6 shadow-sm">
                    <div class="flex justify-between items-center mb-6">
                        ...
                        <span class="text-[9px] font-bold text-emerald-500 bg-emerald-50 px-2 py-0.5 rounded border border-emerald-100 uppercase tracking-tighter shadow-sm">Patching Success</span>
                    </div>
                    ...
                    <div class="text-right text-[9px] font-black text-gray-300 mt-8 border-t pt-4 uppercase tracking-[0.2em]">Live Database</div>
                </div>
                    ...
                    <div class="space-y-4 text-sm flex flex-col">
                        <a href="inventory_list.php?topic=Patching&status=pending" class="block bg-red-50 border-l-4 border-red-500 p-4 rounded-r-lg flex items-start gap-4 text-red-800 transition-all hover:bg-red-100 hover:shadow-md group">
                            ...
                            <div>
                                ...
                                <p class="text-[11px] font-bold opacity-70 mt-0.5"><?= number_format($topic_results[2]['pending'] ?? 0) ?> devices need urgent updates.</p>
                            </div>
                        </a>
                        ...
                        <a href="inventory_list.php?topic=Antivirus&status=pending" class="block bg-orange-50 border-l-4 border-orange-400 p-4 rounded-r-lg flex items-start gap-4 text-orange-800 transition-all hover:bg-orange-100 hover:shadow-md group">
                            ...
                            <div>
                                ...
                                <p class="text-[11px] font-bold opacity-70 mt-0.5"><?= number_format($topic_results[3]['pending'] ?? 0) ?> devices unprotected.</p>
                            </div>
                        </a>
                        ...
                        <a href="inventory_list.php?topic=OS Status&status=pending" class="block bg-blue-50 border-l-4 border-blue-500 p-4 rounded-r-lg flex items-start gap-4 text-blue-800 transition-all hover:bg-blue-100 hover:shadow-md group">
                            ...
                            <div>
                                ...
                                <p class="text-[11px] font-bold opacity-70 mt-0.5"><?= number_format($topic_results[1]['pending'] ?? 0) ?> devices on legacy OS.</p>
                            </div>
                        </a>

                        <!-- NEW: Insight Report Links -->
                        <div class="pt-4 mt-2 border-t border-slate-100 space-y-3">
                            <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest px-1 mb-2">Advanced Insights</p>
                            ...
                            <a href="admin_audit.php" class="flex items-center justify-between p-3 bg-white border border-slate-100 rounded-xl hover:border-rose-300 hover:bg-rose-50 transition-all group shadow-sm">
                                <div class="flex items-center gap-3">
                                    ...
                                    <span class="text-[11px] font-black text-slate-600 group-hover:text-rose-700 uppercase tracking-tighter">Admin Rights Audit</span>
                                </div>
                            ...
                            <a href="patch_insight.php" class="flex items-center justify-between p-3 bg-white border border-slate-100 rounded-xl hover:border-amber-300 hover:bg-amber-50 transition-all group shadow-sm">
                                <div class="flex items-center gap-3">
                                    <span class="text-xl group-hover:rotate-12 transition-transform">📊</span>
                                    <span class="text-[11px] font-black text-slate-600 group-hover:text-amber-700 uppercase tracking-tighter">Patch Detail Insight</span>
                                </div>
                            ...
                            <a href="inactive_assets.php" class="flex items-center justify-between p-3 bg-white border border-slate-100 rounded-xl hover:border-slate-400 hover:bg-slate-50 transition-all group shadow-sm">
                                <div class="flex items-center gap-3">
                                    <span class="text-xl group-hover:rotate-12 transition-transform">💤</span>
                                    <span class="text-[11px] font-black text-slate-600 group-hover:text-slate-800 uppercase tracking-tighter">Inactive Assets Cleanup</span>
                                </div>
        ...
        <footer class="p-6 text-center bg-white border-t border-gray-100 flex flex-col md:flex-row items-center justify-center gap-4">
            <p class="text-[10px] font-black text-gray-400 uppercase tracking-[0.5em]">EIA Dashboard &copy; 2026 Prepared by Endpoint Management Team</p>
