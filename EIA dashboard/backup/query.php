<?php
/**
 * EIA Dashboard - Query V2 (Merged & Optimized)
 * Includes: Advanced Admin Rights Checking (7 patterns) & 90-Day Fresh Patch Analysis
 */
...
/* Missing Critical Patch (90-Day Freshness Filter) */
MissingPatch AS (
    SELECT 
        MS.MACHINE_ID,
...
SELECT 
    ...
    /* OS Build.UBR */
    (SELECT REPLACE(MCI.STR_FIELD_VALUE,'<br/>','')
     FROM MACHINE_CUSTOM_INVENTORY MCI
     WHERE MCI.ID = M.ID AND MCI.SOFTWARE_ID = 65944) AS 'OS Build.UBR',

    OS.os_eos_status AS 'OS End of Support Status',
    
    /* Patch Health Status */
    CASE
        WHEN OS.os_eos_status = 'Ended' THEN 'End of Support'
        WHEN OS.os_eos_status = 'Insider' THEN 'Not Support'
        WHEN MP.MACHINE_ID IS NULL THEN 'Healthy'
        ELSE 'Pending'
    ...
    CASE 
        WHEN (SELECT REPLACE(MCI.STR_FIELD_VALUE,'<br/>','') 
              FROM MACHINE_CUSTOM_INVENTORY MCI
              WHERE MCI.ID = M.ID AND MCI.SOFTWARE_ID = 62583) 
              IN ('Sentinel', 'ESET', 'CrowdStrike', 'Sentinel Agent')
        THEN 'Yes'
        ELSE 'No'
    ...
    CASE 
        WHEN (SELECT REPLACE(MCI.STR_FIELD_VALUE,'<br/>','') 
              FROM MACHINE_CUSTOM_INVENTORY MCI
              WHERE MCI.ID = M.ID AND MCI.SOFTWARE_ID = 62583) LIKE '%Sentinel%' 
        THEN 'Yes'
        WHEN (SELECT REPLACE(MCI.STR_FIELD_VALUE,'<br/>','') 
              FROM MACHINE_CUSTOM_INVENTORY MCI
              WHERE MCI.ID = M.ID AND MCI.SOFTWARE_ID = 69119) 
              = 'Domain=On;Private=On;Public=On' 
        THEN 'Yes'
        ELSE 'No'
    END AS 'Firewall Compliant',

    /* Advanced Admin Rights Verification (7 Approved Patterns) */
    (SELECT REPLACE(MCI.STR_FIELD_VALUE,'<br/>','') 
     FROM MACHINE_CUSTOM_INVENTORY MCI
     WHERE MCI.ID = M.ID AND MCI.SOFTWARE_ID = 42952) AS 'Members of Administrator Group',
                
    CASE 
        WHEN A10003.NAME = 'RIS' THEN 'Yes'
        WHEN (SELECT REPLACE(MCI.STR_FIELD_VALUE,'<br/>','') 
              FROM MACHINE_CUSTOM_INVENTORY MCI
              WHERE MCI.ID = M.ID AND MCI.SOFTWARE_ID = 42952) 
              IN (
    ...
    (SELECT REPLACE(MCI.STR_FIELD_VALUE,'<br/>','') FROM MACHINE_CUSTOM_INVENTORY MCI WHERE MCI.ID = M.ID AND MCI.SOFTWARE_ID = 58066) AS 'BIT Locker Status',
    (SELECT REPLACE(MCI.STR_FIELD_VALUE,'<br/>','') FROM MACHINE_CUSTOM_INVENTORY MCI WHERE MCI.ID = M.ID AND MCI.SOFTWARE_ID = 69119) AS 'Firewall Status',
    (SELECT REPLACE(MCI.STR_FIELD_VALUE,'<br/>','') FROM MACHINE_CUSTOM_INVENTORY MCI WHERE MCI.ID = M.ID AND MCI.SOFTWARE_ID = 68300) AS 'Shared Folder',
    (SELECT REPLACE(MCI.STR_FIELD_VALUE,'<br/>','') FROM MACHINE_CUSTOM_INVENTORY MCI WHERE MCI.ID = M.ID AND MCI.SOFTWARE_ID = 42032) AS 'TLS Settings',
    (SELECT REPLACE(MCI.STR_FIELD_VALUE,'<br/>','') FROM MACHINE_CUSTOM_INVENTORY MCI WHERE MCI.ID = M.ID AND MCI.SOFTWARE_ID = 68808) AS 'USB Storage Permission',
    (SELECT REPLACE(MCI.STR_FIELD_VALUE,'<br/>','') FROM MACHINE_CUSTOM_INVENTORY MCI WHERE MCI.ID = M.ID AND MCI.SOFTWARE_ID = 72225) AS \"Pending Restart\",
