...
if ($needs_update) {
    // FIRST TIME RUN ONLY: Run synchronous query
    ...
    try {
        ...
        file_put_contents($cache_file, json_encode(['ranking' => $bu_data, 'last_updated' => $last_updated]));
    } catch (\PDOException $e) { die("Database Error: " . $e->getMessage()); }
} else {
    // NORMAL LOAD: Load from cache
    $cache_data = json_decode(file_get_contents($cache_file), true);
    $bu_data = $cache_data['ranking'] ?? [];
    $last_updated = $cache_data['last_updated'] ?? 'Pending...';
}
...
<body class="p-4 md:p-6 lg:p-10 flex flex-col items-center">
    ...
    <div class="max-w-5xl w-full glass-card rounded-[2.5rem] p-6 md:p-10 relative overflow-hidden shadow-2xl">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
            ...
            <div class="flex items-center gap-3">
                <div class="text-right hidden md:block">
                    ...
                    <p class="text-[10px] font-black text-slate-500 italic mt-1"><?= $last_updated ?></p>
                </div>
                <a href="index.php" class="bg-blue-600 px-4 py-2.5 rounded-xl shadow-lg border border-blue-500 text-white font-bold flex items-center gap-2 hover:bg-blue-700 transition-all group text-[10px] uppercase tracking-widest active:scale-95">
            ...
            <a href="inventory_list.php?bu=<?= urlencode($bu['bu'] ?? $bu['BU']) ?>" class="block p-3.5 rounded-2xl bg-white/40 border border-white/60 hover:bg-white/80 transition-all duration-300 hover:shadow-xl group">
                <div class="flex justify-between items-center mb-2 px-1">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h3 class="text-[13px] font-black text-slate-800 uppercase tracking-tight group-hover:text-blue-600 transition-colors"><?= htmlspecialchars($bu['bu'] ?? $bu['BU']) ?></h3>
                            <p class="text-[9px] font-bold text-slate-400 uppercase"><?= number_format($total_d) ?> Units</p>
                        </div>
                    </div>
        ...
        <!-- Footer -->
        <footer class="mt-12 text-center pb-4"><p class="text-[9px] font-black text-slate-400 uppercase tracking-[0.5em]">EIA Dashboard &copy; 2026 Prepared by Endpoint Management Team</p></footer>
    </div>
