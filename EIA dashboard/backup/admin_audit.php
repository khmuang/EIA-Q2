...
if ($needs_update || !$data_from_cache) {
    ...
    try {
        $sql = "SELECT `Computer Name`, `Serial no.`, `BU`, `Serviced By` as `Service Team`, `User name` as `Full Name`, `Members of Administrator Group`, `Standard Admin Only` 
                FROM ($clean_query) as base_data 
                WHERE `Standard Admin Only` NOT IN ('Y', 'Yes', 'Compliant', 'Success')";
        $stmt = $pdo->query($sql);
        $all_items = $stmt->fetchAll(PDO::FETCH_ASSOC);
        $last_updated = date("d M Y, H:i");
        file_put_contents($cache_file, json_encode(['items' => $all_items, 'last_updated' => $last_updated]));
    } catch (\PDOException $e) { $all_items = []; $last_updated = "Error"; }
}
    ...
    <div class="max-w-[1600px] mx-auto">
        <div class="glass-card rounded-xl overflow-hidden shadow-2xl border border-white mb-6">
            <div class="p-6 md:p-8 flex flex-col md:flex-row justify-between items-center gap-6 border-b border-white/40 bg-white/30 relative overflow-hidden">
                <div class="absolute inset-0 z-0 opacity-[0.04] pointer-events-none grayscale"><img src="Image/bg-computer-2.jpg" alt="BG" class="w-full h-full object-cover"></div>
                <div class="relative z-10 flex items-center gap-4">
                    ...
                    <div>
                        <h1 class="text-3xl font-black text-slate-800 dark:text-white tracking-tighter" style="font-family: Montserrat;">Standard Admin Rights</h1>
                ...
                <div class="flex flex-wrap items-center gap-3 justify-end relative z-10 text-right">
                    ...
                    <div class="text-[10px] font-black text-rose-600 bg-rose-50 dark:bg-rose-900/20 px-4 py-2 rounded-xl border border-rose-100 dark:border-rose-800/50 shadow-inner uppercase tracking-widest">Found: <?= number_format($total_filtered) ?> Units</div>
                ...
                <table class="w-full text-left whitespace-nowrap border-separate border-spacing-0">
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-700 bg-white/40 dark:bg-slate-800/20">
                        ...
                        <tr class="hover-row transition-all duration-300 group">
                            <td class="px-6 py-4">
                                <a href="inventory_detail.php?name=<?= urlencode($row['Computer Name']) ?>&back=<?= urlencode($_SERVER['REQUEST_URI']) ?>" class="font-black text-slate-800 dark:text-slate-200 hover:text-indigo-600 flex items-center gap-2 transition-colors">
