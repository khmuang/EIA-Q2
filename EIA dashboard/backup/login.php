...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (function_exists('chkldapuser')) {
        ...
        $domains = ['central', 'cmg', 'ofm'];
        $chkldapresult = "Invalid";
        $authenticated_domain = "";
        $full_name = "";

        foreach ($domains as $d) {
            $res = chkldapuser($d, $username_for_ad, $password);
            
            // Handle array response from new adauthen.php
            if (is_array($res) && $res['status'] === "Pass") {
                $chkldapresult = "Pass";
                $authenticated_domain = $d;
                $full_name = $res['full_name'];
                break;
            }
            
            // Support old string response if needed
            if ($res === "Pass") {
                $chkldapresult = "Pass";
                $authenticated_domain = $d;
                break;
            ...
            if ($res !== "Invalid") { $chkldapresult = is_array($res) ? $res['status'] : $res; }
        }
...
<body>
    ...
    <div class="glass-card w-full max-w-lg rounded-[2.5rem] p-10 md:p-14 relative overflow-hidden m-4">
        ...
        <form method="POST" action="login.php" class="space-y-6 relative z-10">
            ...
            <p class="text-center text-[10px] text-gray-500 font-medium uppercase tracking-[0.2em] mt-8">Authorized Personnel Only</p>
        </form>
    </div>
