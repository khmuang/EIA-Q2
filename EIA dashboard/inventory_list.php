        ...
        <div class="glass-card rounded-xl overflow-hidden shadow-2xl relative border border-white">
            ...
            <div class="p-6 md:p-8 flex flex-col md:flex-row justify-between items-center gap-6 border-b border-white/40 bg-white/30 relative overflow-hidden">
                ...
                <div class="flex items-center gap-5 relative z-10">
                    ...
                    <div>
                        <h1 class="text-3xl lg:text-4xl font-black text-slate-800 tracking-tight" style="font-family: 'Montserrat', sans-serif;">EIA Dashboard</h1>
                        <div class="flex items-center gap-2 mt-0.5"><span class="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span><span class="text-xs font-black text-slate-500 uppercase tracking-widest">Live Monitoring From KACE System</span></div>
                    </div>
                </div>
                <div class="flex flex-wrap items-center gap-3 justify-center md:justify-end relative z-10">
                    <div class="flex items-center bg-indigo-50 pl-4 pr-2 py-1.5 rounded-xl border border-indigo-100 shadow-inner group">
                        <span class="text-xs font-black text-indigo-600 uppercase tracking-widest mr-3">Showing <?= number_format($total_rows) ?> Results</span>
            ...
            <form method="GET" class="p-6 bg-slate-50/50 border-b border-slate-200/60">
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-6 gap-4 items-end">
                    <div class="flex flex-col gap-1.5">
                        <label class="text-xs font-black text-slate-400 uppercase tracking-widest ml-1">Search Computer</label>
                    ...
                    <div class="flex flex-col gap-1.5">
                        <label class="text-xs font-black text-slate-400 uppercase tracking-widest ml-1">BU</label>
                    ...
                    <div class="flex flex-col gap-1.5">
                        <label class="text-xs font-black text-slate-400 uppercase tracking-widest ml-1">Service Team</label>
                    ...
                    <div class="flex flex-col gap-1.5">
                        <label class="text-xs font-black text-slate-400 uppercase tracking-widest ml-1">Compliance Score</label>
                    ...
                    <div class="flex gap-2 h-[38px]">
                        <button type="submit" class="flex-1 bg-indigo-600 text-white rounded-xl font-black text-xs uppercase tracking-widest hover:bg-indigo-700 shadow-lg shadow-indigo-200 transition-all border border-indigo-400">Filter</button>
                        <a href="inventory_list.php" class="flex-1 px-4 bg-indigo-50 text-indigo-600 flex items-center justify-center rounded-xl font-black text-xs uppercase tracking-widest border border-indigo-200 hover:bg-indigo-600 hover:text-white transition-all duration-300 shadow-sm group whitespace-nowrap">
                ...
                <div class="mt-5 flex flex-wrap items-center justify-between gap-4">
                    ...
                    <div class="flex flex-wrap items-center gap-2">
                        ...
                        <div class="flex flex-wrap items-center gap-2">
                            <?php if ($bu): ?><span class="text-xs bg-indigo-600 text-white px-3 py-1 rounded-xl shadow-lg shadow-indigo-200 font-black uppercase tracking-tighter border border-indigo-500">BU: <?= htmlspecialchars($bu) ?></span><?php endif; ?>
                            <?php if ($area): ?><span class="text-xs bg-indigo-600 text-white px-3 py-1 rounded-xl shadow-lg shadow-indigo-200 font-black uppercase tracking-tighter border border-indigo-500">Team: <?= htmlspecialchars($area) ?></span><?php endif; ?>
                            <?php if ($topic): ?><span class="text-xs bg-indigo-600 text-white px-3 py-1 rounded-xl shadow-lg shadow-indigo-200 font-black uppercase tracking-tighter border border-indigo-500">Topic: <?= htmlspecialchars($topic) ?></span><?php endif; ?>
                            <?php if ($score_filter): ?><span class="text-xs bg-indigo-600 text-white px-3 py-1 rounded-xl shadow-lg shadow-indigo-200 font-black uppercase tracking-tighter border border-indigo-500">Score: <?= htmlspecialchars($score_filter) ?>%</span><?php endif; ?>
                            <?php if ($search): ?><span class="text-xs bg-amber-500 text-white px-3 py-1 rounded-xl shadow-lg shadow-amber-200 font-black uppercase tracking-tighter border border-amber-400 italic">Search: "<?= htmlspecialchars($search) ?>"</span><?php endif; ?>
                        </div>
                        <?php else: ?>
                        <span class="text-xs text-slate-400 font-bold italic">No active filters applied</span>
                        <?php endif; ?>
                    </div>
                    ...
                    <div class="flex flex-wrap items-center gap-2 bg-white/40 p-1.5 rounded-2xl border border-white/60 shadow-sm">
                        <span class="text-xs font-black text-slate-400 uppercase tracking-widest mx-2">Page Summary:</span>
                        <?php 
                        ...
                        ?>
                        <span class="text-xs bg-emerald-50 text-emerald-700 border border-emerald-100 px-2.5 py-1 rounded-lg font-black uppercase tracking-tighter flex items-center gap-1.5"><div class="w-1.5 h-1.5 bg-emerald-500 rounded-full"></div>Full: <?= $page_full ?></span>
                        <span class="text-xs bg-rose-50 text-rose-700 border border-rose-100 px-2.5 py-1 rounded-lg font-black uppercase tracking-tighter flex items-center gap-1.5"><div class="w-1.5 h-1.5 bg-rose-500 rounded-full"></div>Critical: <?= $page_crit ?></span>
                    </div>
                </div>
            </form>
                        ...
                        <tr class="hover-row transition-all duration-300 group">
                            ...
                            <td class="px-6 py-4 text-center">
                                <div class="flex flex-col items-center gap-1.5">
                                    <div class="inline-flex px-2 py-1 rounded-lg border font-black text-xs <?= $bg_soft ?> <?= $t_color ?> <?= $border_c ?> shadow-sm min-w-[45px] justify-center"><?= $score_pct ?>%</div>
