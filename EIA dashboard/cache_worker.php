...
try {
    ...
    switch ($type) {
        ...
        case 'bu_ranking':
            log_message("Updating bu_ranking and service_team_ranking...");
            ...
            $sql_bu = "SELECT BU, 
                    ...
                    GROUP BY BU
                    ORDER BY (SUM(CASE WHEN `Joined Approved Domain` = 'Yes' THEN 1 ELSE 0 END +
                        CASE WHEN `OS End of Support Status` = 'Active' THEN 1 ELSE 0 END +
                        CASE WHEN `Patch Healthy` = 'Healthy' THEN 1 ELSE 0 END +
                        CASE WHEN `Antivirus Compliant` = 'Yes' THEN 1 ELSE 0 END +
                        CASE WHEN `Firewall Compliant` = 'Yes' THEN 1 ELSE 0 END +
            ...
            $bu_ranking = $pdo->query($sql_bu)->fetchAll(PDO::FETCH_ASSOC);

            // 2. Service Team Ranking Query
            $sql_team = "SELECT `Serviced By` as team, 
                    COUNT(*) as total_devices,
                    SUM(CASE WHEN `Joined Approved Domain` = 'Yes' THEN 1 ELSE 0 END +
                        CASE WHEN `OS End of Support Status` = 'Active' THEN 1 ELSE 0 END +
                        CASE WHEN `Patch Healthy` = 'Healthy' THEN 1 ELSE 0 END +
            ...
            $team_ranking = $pdo->query($sql_team)->fetchAll(PDO::FETCH_ASSOC);
            ...
            log_message("BU and Team ranking cache updated.");
            break;
    }
