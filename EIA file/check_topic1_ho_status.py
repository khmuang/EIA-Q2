...
for sheet_name, header_idx in sheet_configs.items():
    ...
    if group_col and status_col:
        # HO Filter
        ho_df = df[df[group_col].astype(str).str.upper().str.contains('HO', na=False)]
        dist = ho_df[status_col].astype(str).str.strip().str.upper().value_counts()
        for val, count in dist.items():
            print(f"{sheet_name:<15} | HO     | {val:<10} | {count}")
